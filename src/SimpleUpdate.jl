...
struct SimpleUpdateProblem{TH,TA,TW}
    h::TH
    a::TA
    b::TA
    σab::TW
...
"""
    simpleupdate(h, ainit, binit)
creates an object of type `SimpleUpdateProblem` that contains the hamiltonian `h`,
...
"""
function simpleupdate(h, ainit, binit)
    a, b, σab, σba, ωab, ωba = extract_weights!(deepcopy(ainit),deepcopy(binit))
    apply!.((σab, σba, ωab, ωba), x -> x .= x ./ norm(x))
    return SimpleUpdateProblem{typeof(h), typeof(a), typeof(σab)}(h, a, b, σab, σba, ωab, ωba)
end
...
function update!(su::SimpleUpdateProblem)
    @unpack a, b, σab, σba, ωab, ωba, h = su

    update!((a,b,σab), (σba, ωab, ωba, h))
    update!((b,a,σba), (σab, ωab, ωba, h))

    arot = tensorcopy(a,(1,2,3,4,5), (4,1,2,3,5))
    brot = tensorcopy(b,(1,2,3,4,5), (4,1,2,3,5))
    update!((arot, brot, ωab), (ωba, σab, σba, h))
    update!((brot, arot, ωba), (ωab, σab, σba, h))
